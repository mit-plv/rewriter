...
let rewriter_path =
  DirPath.make (List.map Id.of_string ["RewriterBuild";"plugins";"Util";"Rewriter"])
...
let ltac_call tac (args:glob_tactic_arg list) =
  TacArg(CAst.make @@ TacCall (CAst.make (ArgArg(Loc.tag @@ tac),args)))
...
let rewriter_heuristic_process_rules_proofs env sigma ~poly rules_proofs =
  ...
  let (rules_proofs_sig, _uses_unsafe_tactic, uctx) =
    Pfedit.build_by_tactic env uctx ~poly rules_proofs_with_args rules_proofs_with_args_tac in
...
let rewriter_scrape_data env sigma ~poly (rules_proofs : EConstr.t) =
  ...
  let scraped_data_ty = mkApp (scraped_data_with_args, [|rules_ty; rules_proofs|]) in
  let uctx = Evd.evar_universe_context sigma in
  let (scraped_data, _uses_unsafe_tactic, uctx) =
    Pfedit.build_by_tactic env uctx ~poly scraped_data_ty scraped_data_with_args_tac in
  let sigma = Evd.from_ctx uctx in (* XXX IS THIS RIGHT? *)
...
let rewriter_emit_inductives sigma ~poly (scraped_data : EConstr.t) (base_name : Names.Id.t option) (ident_name : Names.Id.t option) (raw_ident_name : Names.Id.t option) (pattern_ident_name : Names.Id.t option) =
  ...
  let uctx = Evd.evar_universe_context sigma in
  let (base_elim, _uses_unsafe_tactic, uctx) =
    Pfedit.build_by_tactic env uctx ~poly base_elim_ty base_elim_with_args_tac in
  let sigma = Evd.from_ctx uctx in (* XXX IS THIS RIGHT? *)
  ...
  let uctx = Evd.evar_universe_context sigma in
  let (ident_elim, _uses_unsafe_tactic, uctx) =
    Pfedit.build_by_tactic env uctx ~poly ident_elim_ty ident_elim_with_args_tac in
  let (pattern_ident_elim, _uses_unsafe_tactic, uctx) =
    Pfedit.build_by_tactic env uctx ~poly pattern_ident_elim_ty pattern_ident_elim_with_args_tac in
  let (raw_ident_elim, _uses_unsafe_tactic, uctx) =
    Pfedit.build_by_tactic env uctx ~poly raw_ident_elim_ty raw_ident_elim_with_args_tac in
  let sigma = Evd.from_ctx uctx in (* XXX IS THIS RIGHT? *)
...
let make_rewriter_of_scraped_and_ind sigma ~poly package_name scraped_data var_like_idents base ident raw_ident pattern_ident include_interp rules_proofs =
  let env = Global.env () in
  let (sigma, verified_rewriter_with_args) = Evd.fresh_global env sigma (r_verified_rewriter_with_args ()) in

  let (sigma, verified_rewriter_with_args) = Evd.fresh_global env sigma (r_verified_rewriter_with_args ()) in
  let rules_ty = rules_ty_of_rules_proofs env sigma rules_proofs in
  let verified_rewriter_ty = mkApp (verified_rewriter_with_args, [|scraped_data; var_like_idents; base; ident; raw_ident; pattern_ident; include_interp; rules_ty; rules_proofs|]) in
  ...
  let (sigma, verified_rewriter_name) = make_definition_by_tactic sigma ~poly (Some package_name) verified_rewriter_ty verified_rewriter_with_args_tac in
...
let make_rewriter sigma ~poly ~var_like_idents ~include_interp (package_name : Names.Id.t) (rules_proofs : EConstr.t) =
  let env = Global.env () in
  ...
  let (sigma, scraped_data) = rewriter_scrape_data env sigma ~poly rules_proofs in
  let (sigma, (base, ident, raw_ident, pattern_ident)) = rewriter_emit_inductives sigma ~poly scraped_data None None None None in
  let (sigma, rewriter) = make_rewriter_of_scraped_and_ind sigma ~poly package_name scraped_data var_like_idents base ident raw_ident pattern_ident include_interp rules_proofs in
  (sigma, rewriter)
...
let vernac_make_rewriter ~poly ?(var_like_idents = None) ?(include_interp = false) (package_name : Names.Id.t) (rules_proofs : Constrexpr.constr_expr) =
  let env = Global.env () in
  let sigma = Evd.from_env env in
  ...
  ignore(make_rewriter sigma package_name rules_proofs ~poly ~var_like_idents ~include_interp)
