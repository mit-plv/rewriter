...
Module Compilers.
  ...
  Module expr.
    ...
    Ltac reify_in_context base_type ident reify_base_type reify_ident var term value_ctx template_ctx :=
      ...
      res.
    Ltac reify base_type ident from_flat reify_base_type reify_ident var term :=
      let pvar := constr:(fun _ : type.type base_type => PositiveMap.key) in
      let v := reify_in_context base_type ident reify_base_type reify_ident pvar term (@var_context.nil base_type pvar) tt in
      let t := type of v in
      let t := lazymatch (eval hnf in t) with @expr.expr _ _ _ ?t => t end in
      let v := (eval vm_compute in (@GeneralizeVar.to_flat base_type ident t v)) in
      constr:(from_flat t v var).
    Ltac Reify base_type ident from_flat reify_base_type reify_ident term :=
      constr:(fun var : type base_type -> Type
              => ltac:(let r := reify base_type ident from_flat reify_base_type reify_ident var term in
                       exact r)).
    Ltac Reify_rhs base_type ident from_flat reify_base_type reify_ident base_interp interp_ident _ :=
      let RHS := lazymatch goal with |- _ = ?RHS => RHS end in
      let R := Reify base_type ident from_flat reify_base_type reify_ident RHS in
